...
def main():
    ...
    for bp_path in BLUEPRINTS:
        ...
        print(f'Trinity: T={t_audit["transitions"]["count"]} G={t_audit["gates"]["count"]} A={t_audit["actions"]["count"]}')
        
        # Audit Flange
        flange = auditFlange({'blueprint': bp})
        f_audit = flange['flangeAudit']
        print(f'Flange: props={f_audit["properties"]["count"]} hermeticity={f_audit["hermeticity"]["score"]:.2f}')
        ...
        print(f'TLC: passed={tlc.get("passed")} states={tlc.get("statesExplored", 0)}')
        
        # Run TLAPS (or simulate)
        tlaps_result = runTlaps({'tlaPath': tla_result.get('tlaPath')})
        tlaps = tlaps_result.get('tlapsResult', {})
        print(f'TLAPS: passed={tlaps.get("passed")} theorems={list(tlaps.get("theorems", {}).keys())}')
    ...
    for cert in all_certs:
        bp_info = cert['blueprint']
        print(f'| {bp_info["id"]} | {bp_info["version"]} | {cert["seal"]} | {cert["level"]} | {bp_info["hash"][:16]} |')
