...
def generate_compute_unit_tests(
    ...
    lines = [
        '"""',
        f'Compute Unit Tests for {blueprint.get("id", "unknown")}',
        '',
    ...
    for idx, cu in enumerate(compute_units, 1):
        ...
        lines.extend([
            ...
            '',
            f'    def test_{func_name}_input_contract(self, mock_context):',
        ...
        lines.extend([
            '        }',
            f'        # TODO: Import and call the actual compute function',
            f'        # from {module_prefix}.compute import {func_name}',
        ...
        lines.extend([
            '',
            f'    def test_{func_name}_output_contract(self, mock_context):',
            f'        """Test that {func_name} returns expected output fields."""',
            f'        expected_outputs = {list(output_map.keys())}',
            '',
            ...
            '',
            f'    def test_{func_name}_error_handling(self, mock_context):',
            ...
            f'        # assert "error" in result or result is None',
            '',
            '        # Placeholder',
            '        assert True',
            '',
        ...
        # Generate boundary tests for each input
        for inp_key, ctx_key in input_map.items():
            prop_spec = properties.get(ctx_key, {})
            prop_type = prop_spec.get("type", "string")

            lines.extend([
                f'    def test_{func_name}_{inp_key}_null(self, mock_context):',
            ...
            lines.extend([
                '        }',
                '',
                f'        # Compute should handle null {inp_key}',
...
def generate_comprehensive_tests(
    ...
    if compute_units:
        compute_pytest = generate_compute_unit_tests(bp, compute_units)
