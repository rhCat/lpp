...
def sanitize_python_code(code: str, filename: str = "code.py", verbose: bool = False) -> Tuple[str, List[str]]:
    """
    ...
    if fix_count > 0:
        ...
        if verbose:
            print(f"  [SANITIZE] {filename}: Fixed {fix_count} literal newlines")
    ...
    try:
        ...
        if verbose:
            print(f"  [SANITIZE] {filename}: Still has syntax error after fixes: {e.msg}")
...
def _fix_literal_newlines_in_strings(code: str) -> Tuple[str, int]:
    ...
    while i < len(lines):
        ...
        # Look for lines ending with unclosed quotes in print, f-string, or assignment
        ...
        if unclosed_match:
            ...
            while j < len(lines) and not found_close:
                ...
                if _closes_string(stripped, quote_char, is_fstring):
                    ...
                    fix_count += 1
                    j += 1
                elif _could_be_string_continuation(stripped, quote_char, is_fstring):
...
def _could_be_string_continuation(line: str, quote: str, is_fstring: bool) -> bool:
    ...
    keywords = ['def ', 'class ', 'if ', 'for ', 'while ', 'return ', 'import ', 'from ']
...
def validate_python_file(file_path: str, verbose: bool = False) -> Tuple[bool, List[str]]:
    """
...
def validate_skill_directory(
    ...
    for compute_file in compute_files:
        ...
        if struct_error:
            ...
            results["results"][compute_file.name]["errors"].append(struct_error)
            if verbose:
                print(f"  [STRUCTURE] {compute_file.name}: FAIL - {struct_error}")
        elif verbose:
    ...
    if interactive_file.exists():
        import_errors = _check_imports(interactive_file, src_dir, compute_files)
        if import_errors:
            results["passed"] = False
            results["errors"].extend(import_errors)
            results["results"]["interactive.py"]["errors"].extend(import_errors)
            if verbose:
                for err in import_errors:
                    print(f"  [IMPORT] interactive.py: FAIL - {err}")
        elif verbose:
            print(f"  [IMPORT] interactive.py: PASS")
...
def _check_content_issues(content: str, filename: str) -> List[str]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if stripped.endswith(('print("', "print('", '= "', "= '", 'f"', "f'", 'input("', "input('")):
            if i < len(lines):
                next_line = lines[i].strip() if i < len(lines) else ""
                if next_line and not next_line.startswith(('#', 'def ', 'class ', 'if ', 'for ', 'while ', '"""', "'''")):
                    issues.append(f"Line {i}: Possible unclosed string literal")
    ...
    placeholders = ['# TODO', '# FIXME', '# Add code here', '# Implement', 'pass  #']
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id == "COMPUTE_UNITS":
...
def _check_imports(
    ...
    try:
        ...
        if "COMPUTE_UNITS" in content and not compute_files:
            errors.append("References COMPUTE_UNITS but no compute module found")
...
def _build_feedback(results: Dict[str, Any]) -> str:
    ...
    if results["passed"]:
        return f"Operational validation PASSED: {results['files_checked']} files valid"
    ...
    if "EOL while scanning string literal" in all_errors or "unterminated string" in all_errors:
        feedback_parts.append("")
        feedback_parts.append("FIX REQUIRED: You have literal newlines inside strings.")
        feedback_parts.append("WRONG: print(\"Hello")
        feedback_parts.append("       World\")")
        feedback_parts.append("RIGHT: print(\"Hello\\nWorld\")")
        feedback_parts.append("Use \\n for newlines inside strings, NOT actual line breaks!")
    
    if "Missing COMPUTE_UNITS" in all_errors:
        feedback_parts.append("")
        feedback_parts.append("FIX REQUIRED: Compute module must export COMPUTE_UNITS dict.")
        feedback_parts.append("Add at bottom: COMPUTE_UNITS = {\"process\": process_func, ...}")
    
    if "Missing module" in all_errors:
        feedback_parts.append("")
        feedback_parts.append("FIX REQUIRED: interactive.py imports a module that doesn't exist.")
        feedback_parts.append("Ensure src/ contains the compute module referenced in import.")
